...
pub struct Event {
    ...
    /// Time of the event. None = all-day event (sorted last).
...
impl MonthData {
    /// Get events for a specific day, sorted by time (all-day events last).
    pub fn events_for_day(&self, day: u32) -> Vec<&Event> {
        ...
        events.sort_by_key(|e| match e.time {
            Some(t) => (0, t),
        ...
        events_by_day.insert(
            ...
            vec![
                ...
                Event {
                    ...
                    time: NaiveTime::from_hms_opt(17, 0, 0),
                    date: date_20,
                },
            ],
        );
...
mod tests {
    ...
    fn test_events_sorted_by_time() {
        ...
        assert!(events.last().unwrap().time.is_none());
    }
